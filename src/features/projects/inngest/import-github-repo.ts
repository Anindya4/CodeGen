...
interface ImportGithubRepoEventProps {
  owner: string;
  repo: string;
...
export const ImportGithubRepo = inngest.createFunction(
  {
    ...
    onFailure: async ({ event, step }) => {
      ...
      const { projectId } = event.data.event.data as ImportGithubRepoEventProps;
  ...
  async ({ event, step }) => {
    const { owner, repo, projectId, githubToken } = event.data as ImportGithubRepoEventProps;
    const internalKey = process.env.CODEGEN_CONVEX_INTERNAL_KEY;
    if (!internalKey) {
    ...
    //start cleaning up any existing files in the project
    await step.run("cleanup-project", async () => {
      await convex.mutation(api.system.cleanUpImport, {
        internalKey,
        projectId
      });
    });
    ...
    //this makes sure that the folder creates before any file (if a file is inside of a folder)
    ...
    const folderIdMap = await step.run("create-folders", async () => {
      ...
      for (const folder of folders) {
        ...
        const createFolder = await convex.mutation(api.system.createFolder, {
          internalKey,
          projectId,
          name,
          parentId,
        ...
        map[folder.path] = createFolder;
      }
      return map;
    });


    // get all files form the tree
    ...
    await step.run("create-files", async () => {
      for (const file of allFiles) {
        ...
        try {
          ...
          console.error(`Failed to import files: ${file.path}`)
        };
      }
    });
