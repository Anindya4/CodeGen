...
//function for getting file path in the editor:
...
//function to getting/fetching content of a folder:
...
export const createFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    //check if file with same name already exists or not:
    const files = await ctx.db
      .query("files")
      .withIndex("by_project_parent", (q) =>
        q.eq("projectId", args.projectId).eq("parentId", args.parentId),
...
export const createFolder = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    //check if folder with same name already exists or not:
    const files = await ctx.db
      .query("files")
      .withIndex("by_project_parent", (q) =>
        q.eq("projectId", args.projectId).eq("parentId", args.parentId),
  ...
  handler: async (ctx,args) => {
    ...
    .withIndex("by_project_parent", (q) =>
      q
        ...
        .eq("parentId", file.parentId)).collect()
...
export const deleteFile = mutation ({
  ...
  handler: async (ctx,args) => {
    ...
    const deleteRecursive = async (fileId: Id<"files">) => {
      ...
      //delete storage file if exists(photos or anything else)
      if (item.storageId) {
        await ctx.storage.delete(item.storageId);
      }

      await ctx.db.delete("files", fileId) //deletes the file/folder itself
    };
